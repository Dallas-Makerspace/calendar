...
<div class="events view">
    ...
    <?php if (in_array($event->status, ['cancelled', 'rejected'])) { ?>
        <div class="alert alert-danger">
            <strong>Notice!</strong> This event has been cancelled. Registrations have been automatically cancelled and
        ...
        <div class="col-sm-5">
            ...
            <ul class="list-unstyled">
                ...
                <?php if ($event->eventbrite_link) { ?>
                    <li><strong>Eventbrite Registration:</strong> <a href="<?= $event->eventbrite_link ?>"
                                                                     target="_blank">Complete required third party
                            registration</a></li>
                    <li><strong>Considerations:</strong> This event's required fee is not processed through the DMS
                        payment system and is completed through the host's Eventbrite account. Due to this, the host is
                        responsible for any refunds that may be needed for this event.
                    </li>
                <?php } ?>
            ...
            <?php if ($cutoff > $now && $event->status == 'approved') { ?>
                <?php if ($hasOpenSpaces) { ?>
                    ...
                    if (is_int($openSpaces)) {
                        ?><p class="spaces_avaliable"><?= $openSpaces ?> spaces of <?= $totalSpaces ?>
                        available</p><?php
                    }
                    ?>
                <?php } else { ?>
                ...
                <div>
                    ...
                    <?php foreach ($addToCalLinks as $link) { ?>
                        ...
                        <a target="_blank" href="<?= $link['url'] ?>">
                            <img alt="<?= $link['hint'] ?>" src="/img/<?= $link["icon"] ?>">
                        </a>
                    </span>
                    <?php } ?>
                </div>
            ...
            <div>
                ...
                foreach ($event->files as $file) {
                    if (!$file->private || $canManageEvents || $event->created_by == $authUsername) {
                        ...
                        $imageEnding = $exploded[count($exploded) - 1];
...
<?php if ($canManageEvents || $event->created_by == $authUsername) { ?>
    ...
    <div>
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" class="active"><a href="#registrations" aria-controls="registrations" role="tab"
                                                      data-toggle="tab">Registrations <span
                        class="badge"><?= (isset($event->registrations) && !empty($event->registration)) ? count($event->registrations) : "0"; ?></span></a>
            </li>
            <li role="presentation"><a href="#attendance" aria-controls="attendance" role="tab" data-toggle="tab">Attendance</a>
            </li>
            <?php if ($event->fulfills_prerequisite_id) { ?>
                <li role="presentation"><a href="#assignments" aria-controls="assignments" role="tab" data-toggle="tab">AD
                        Assignment</a></li>
            <?php } ?>
        </ul>
        ...
        <div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane" id="attendance">
                ...
                <?php } else { ?>
                    <?php if ($time > $now) { ?>
                        <p class="text-right">Attendance can be taken at the start of the class through the following 24
                            hours.</p>
                    <?php } else { ?>
            ...
            <?php if ($event->fulfills_prerequisite_id) { ?>
                <div role="tabpanel" class="tab-pane" id="assignments">
                    <div class="alert alert-warning" role="alert">
                        <strong>Caution!</strong> AD Permissions can not be removed once given. Double check your
                        assignments before submitting. If you do make a mistake the group will need to be remove
                        manually in Active Directory.
                    </div>
